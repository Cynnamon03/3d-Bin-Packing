...
(x, y, z) coordinates are derived by DBLF, not encoded directly.
...
from geometry_3d import (
    N_ORIENTATIONS, get_dims,
    place_bin_dblf,
    assign_weights, compute_weight_capacity,
...
class Wolf3D:
    ...
    def __init__(self, items, container, weight_cap, m=None):
        ...
        self.m          = m or max(2, math.ceil(vol_items / vol_cap) * 2)
    ...
    def _make_random_genes(self):
        ...
        Ensures roughly n/m items per bin, avoiding the bug where
        volume-greedy assignment puts all items in one bin when
        total_volume is close to container_volume.
        """
    ...
    def check_and_correct(self):
        """
        Reassign overflow items to bins with remaining volume,
        or open new bins. Uses a volume-based estimate (fast).
        """
        if self.overflow == 0:
            return

        cap = (self.container['L'] *
               self.container['H'] *
               self.container['D'])

        # Volume used per bin from current genes
        bin_vol = {}
        for i, b in enumerate(self.genes):
            vol = (self.items[i]['L'] *
                   self.items[i]['H'] *
                   self.items[i]['D'])
            bin_vol[b] = bin_vol.get(b, 0) + vol

        # Re-detect overflow items
        groups = {}
        for i, b in enumerate(self.genes):
            groups.setdefault(b, []).append(i)
        orient_map = {i: self.orients[i] for i in range(self.n)}
        ...
        for item_idx in overflow_items:
            item_vol = (self.items[item_idx]['L'] *
                        self.items[item_idx]['H'] *
                        self.items[item_idx]['D'])

            # Find bin with most remaining volume (1.2x buffer for geometry)
            target      = None
            best_remain = 0
            for b in sorted(bin_vol):
                remaining = cap - bin_vol.get(b, 0)
                if remaining >= item_vol * 1.2 and remaining > best_remain:
                    target      = b
                    best_remain = remaining

            if target is not None:
                self.genes[item_idx]  = target
                bin_vol[target]      += item_vol
            else:
                new_b               = max(self.genes) + 1
                self.m              = max(self.m, new_b + 1)
                self.genes[item_idx] = new_b
                bin_vol[new_b]       = item_vol
...
def simulated_annealing_3d(wolf, T0=500.0, delta_T=25.0,
    ...
    while T > freeze:
        for _ in range(max_process):
            ng, no, name = apply_udhc_3d(current.genes, current.orients, current.m)
            cand         = current.copy()
            cand.genes   = ng
            cand.orients = no
            cand._decode()
...
def hd_gwo_3d(items, container,
    ...
    for it in range(max_iter):
        if time.time() - t0 > max_time:
            print(f"  [iter {it+1}] Time limit.", file=sys.stderr, flush=True)
            break
